...
class UsersController extends AppController {
    ...
    /*public function mobile_add() {
        ...
        $tt = $this->User->find("count", array('conditions' => array("username" => $_POST['UserName'])));
        if ($tt == 0) {
            $result["result"] = "Available";
        } else {
            $result["result"] = "Not Available";
        }
        $this->set('test', json_encode($_POST));
    }*/
    public function mobile_add() {
        $this->layout = "";
        Configure::write('debug', 0);
        //$json='{"Registeration_Data":{"EmailAdress":"mail@gmail.com","Password":"123456","FullName":"mohamed faroon","MobileNo":"1234567890"}}';
        ...
        /* if (!empty($this->data)) {
          ...
          $roles = $this->User->Role->find('list');
          $this->set(compact('roles')); */
        /* $rr= json_encode($_POST['Authentication']);
          $tt=json_encode($_POST );
          $tt= json_decode($tt);
          $tt= json_decode($_POST['Authentication']); */


    
            $this->User->create();
            $this->data['User']['activation_key'] = md5(uniqid());
            $this->data['User']['username'] = $_POST["EmailAddress"];
            $this->data['User']['password'] = $this->Auth->password($_POST["Password"]);
            $this->data['User']['name'] = $_POST["FullName"];
            $this->data['User']['email'] = $_POST["EmailAddress"];
            $this->data['User']['mobileno'] = $_POST["MobileNo"];
            $this->data['User']['role_id'] = 2;
            $this->data['User']['status'] = 1;
            $tt = $this->User->find("count", array('conditions' => array("username" => $_POST['EmailAddress'])));
            if ($tt == 1) {
    ...
    public function check_login() {
        ...
        $re=$this->User->find("all", array('conditions' => array("username" => $_POST['UserName'], 'password' => $this->Auth->password($_POST['Password']))));
        //$tt = $this->User->find("count", array('conditions' => array("username" => $_POST['UserName'], 'password' => $this->Auth->password($_POST['Password']))));
        if ($re) {
            
            $result["result"] = "Exist";
            $result["MobileNo"] = $re[0]['User']['mobileno'];
            $result["UserName"] = $re[0]['User']['username'];
            $result["FullName"] = $re[0]['User']['name'];
        } else {
